...
const DUMMY_ASSIGNMENTS = [
  { id: '1', subject: '중2-방정식 기초', timeLimit: '20분', assignedAt: '2026-03-14', status: 'pending' },
  { id: '2', subject: '중2-연립방정식 응용', timeLimit: '20분', assignedAt: '2026-03-10', status: 'done' },
];
...
function StudentContent() {
  ...
  const pending = DUMMY_ASSIGNMENTS.filter((a) => a.status === 'pending');
  const done = DUMMY_ASSIGNMENTS.filter((a) => a.status === 'done');
    ...
    <div className="min-h-screen bg-gray-50">
      <main className="max-w-xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
          ...
          <h1 className="text-xl font-bold text-gray-900">{userId} 학생</h1>
          <p className="text-sm text-gray-500 mt-1">오늘도 열심히 해봐요!</p>
        </div>
        ...
        <div>
          ...
          {pending.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-200 p-6 text-center text-sm text-gray-400">배정된 시험이 없습니다.</div>
          ) : (
