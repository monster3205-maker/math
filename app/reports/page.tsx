...
function GradeTabs({ activeTab, availableGrades, onChange, counts }: {
  activeTab: GradeTab;
  availableGrades: string[];
  onChange: (tab: GradeTab) => void;
  counts: Record<string, number>;
}) {
  const tabs: GradeTab[] = ['전체', ...GRADE_ORDER.filter((g) => availableGrades.includes(g))];
  return (
...
function createEmptyRecord(): LessonRecord {
  return { id: crypto.randomUUID(), date: new Date().toISOString().slice(0, 10), studentName: '', grade: '', studyUnit: '', notes: '' };
}
...
function ReportsContent() {
  ...
  const generateReports = async () => {
    ...
    if (filtered.length === 0) { alert(`${selectedYear}년 ${selectedMonth}월에 해당하는 수업 기록이 없습니다.`); return; }

    const groups = new Map<string, { grade: string; records: LessonRecord[] }>();
    for (const r of filtered) {
      if (!r.studentName.trim()) continue;
      if (!groups.has(r.studentName)) groups.set(r.studentName, { grade: r.grade, records: [] });
      groups.get(r.studentName)!.records.push(r);
    }
    if (groups.size === 0) { alert('학생명이 입력된 기록이 없습니다.'); return; }

    setIsLoading(true); setErrors({}); setReports([]); setReportTab('전체');
    ...
    await Promise.all(
      Array.from(groups.entries()).map(async ([studentName, { grade, records: studentRecords }]) => {
        const body: GenerateReportRequest = {
          studentName, grade, month: selectedMonth, year: selectedYear,
        ...
        try {
          ...
          newReports.push({ ...data, studentName, grade, month: selectedMonth, year: selectedYear, records: body.records, provider });
        } catch (e) {
    ...
    setReports(newReports); setErrors(newErrors); setIsLoading(false);
    if (newReports.length > 0) setTimeout(() => document.getElementById('reports-section')?.scrollIntoView({ behavior: 'smooth' }), 100);
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        <section className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex rounded-lg border border-gray-300 overflow-hidden text-sm font-medium">
              <button onClick={() => setProvider('gpt')} className={`px-4 py-2 transition-colors ${provider === 'gpt' ? 'bg-green-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>GPT-4o</button>
              <button onClick={() => setProvider('claude')} className={`px-4 py-2 transition-colors border-l border-gray-300 ${provider === 'claude' ? 'bg-orange-500 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>Claude</button>
            </div>
            <div className="w-px h-6 bg-gray-200" />
            <select value={selectedYear} onChange={(e) => setSelectedYear(Number(e.target.value))} className="px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400">
              {years.map((y) => <option key={y} value={y}>{y}년</option>)}
            </select>
            <select value={selectedMonth} onChange={(e) => setSelectedMonth(Number(e.target.value))} className="px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400">
              {Array.from({ length: 12 }, (_, i) => i + 1).map((m) => <option key={m} value={m}>{m}월</option>)}
            </select>
            <button onClick={generateReports} disabled={isLoading} className="px-6 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white text-sm font-semibold rounded-lg transition-colors">
        ...
        <section className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          ...
          <GradeTabs activeTab={inputTab} availableGrades={GRADE_ORDER} onChange={setInputTab} counts={inputCounts} />
          <InputTable records={filteredRecords} onAdd={addRow} onDelete={deleteRow} onChange={updateRow} />
        </section>

        {Object.keys(errors).length > 0 && (
          <section className="bg-red-50 border border-red-200 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-red-700 mb-2">생성 오류</h2>
            <ul className="space-y-1">{Object.entries(errors).map(([name, msg]) => <li key={name} className="text-sm text-red-600">{name}: {msg}</li>)}</ul>
          </section>
        )}
        ...
        {reports.length > 0 && (
          <section id="reports-section" className="space-y-6">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <h2 className="text-base font-semibold text-gray-800">{selectedYear}년 {selectedMonth}월 보고서 ({filteredReports.length}/{reports.length}명)</h2>
            </div>
            <GradeTabs activeTab={reportTab} availableGrades={GRADE_ORDER} onChange={setReportTab} counts={reportCounts} />
            <div className="space-y-8">{filteredReports.map((report) => <ReportCard key={report.studentName} report={report} />)}</div>
          </section>
        )}
      </main>
    </div>
